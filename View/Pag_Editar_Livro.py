...
class Pag_Editar_Livro():
  def __init__(self, container, controller):
    ...
    self.labelCadastro = tk.Label(self.retangulo, width=20, text='Editar livro', bg = "#545454", font=("Arial", 25))
  ...
  def enviar(self):
    ...
    self.resultado = self.controller.editandoLivro(dadosLivro)
    self.mensagem()

  def mensagem(self):
    if self.resultado:
      messagebox.showinfo('Informação', "Livro editado com sucesso")
      self.controller.showFrame5()
    else:
      messagebox.showerror('Erro', 'Falha ao editar Livro.') 
